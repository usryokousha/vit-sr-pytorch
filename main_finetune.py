...
from SimMIM.utils import load_checkpoint, load_pretrained, save_checkpoint, get_grad_norm, auto_resume_helper, reduce_tensor

from vit_sr_pytorch.data import JensenShannonLoss
...
def train_one_epoch(config, model, criterion, data_loader, optimizer, scaler, epoch, mixup_fn, lr_scheduler, writer):
    model.train()
    optimizer.zero_grad()
...
def validate(config, data_loader, model, writer, global_step):
    ...
    writer.add_scalar('val/acc5', acc5_meter.avg, global_step)
