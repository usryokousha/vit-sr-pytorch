...
from vit_sr_pytorch.dist import get_world_size, all_reduce
...
def load_checkpoint(config, model, optimizer, lr_scheduler, scaler, logger):
    ...
    logger.info(msg)
    max_accuracy = 0.0
    if not config.EVAL_MODE and 'optimizer' in checkpoint and 'lr_scheduler' in checkpoint and 'epoch' in checkpoint and 'scaler' in checkpoint:
...
def reduce_tensor(tensor):
    rt = tensor.clone()
    all_reduce(rt, op=dist.ReduceOp.SUM)
    rt /= get_world_size()
    return rt
...
def load_vae(config, model_class, logger):
    logger.info(f">>>>>>>>>> Trained from {config.VAE} ..........")
    checkpoint = torch.load(config.VAE, map_location='cpu')
    checkpoint_model = checkpoint['weights']
    hparams = checkpoint['hparams']
    
    # Only load encoder parameters and codebook
    if config.MODEL.VAE.ENCODER_ONLY:
        checkpoint_model = {k: v for k, v in checkpoint_model.items() if k.startswith('encoder.') or k.startswith('codebook.')}
        logger.info('Loaded only encoder and codebook parameters.')
    ...
    del checkpoint
    torch.cuda.empty_cache()
    logger.info(f">>>>>>>>>> loaded successfully '{config.VAE}'")
    return model
...
def pixel_accuracy(output: torch.Tensor, target: torch.Tensor, dim: int = 1):
    """Computes the accuracy of predictions over all pixels"""
    _, pred = output.max(dim=dim)
    correct = pred == target
    accuracy = correct.sum().float() / target.numel()
    return accuracy